...
let buffer_name = fun stream ->
  ...
  | Some span -> "__riot_macro_format_buffer_"
  ^ string_of_int span.start
  ^ "_"
  ^ string_of_int span.end_
...
let render_builder_program = fun ~buffer_name ~capacity format_items values ->
  ...
  let rec collect statements_rev remaining_values = function
    ...
    | Macro_format_parser.String text :: rest ->
        collect (append (render_literal_source text) statements_rev) remaining_values rest
    | Macro_format_parser.Hole Macro_format_parser.Next_arg_to_string :: rest -> (
        match remaining_values with
        | value :: remaining -> collect (append value statements_rev) remaining rest
        | [] -> Error "format! placeholder count does not match the number of supplied arguments"
      )
    | Macro_format_parser.Hole (Macro_format_parser.Var_to_string name) :: rest ->
        collect (append ("(" ^ name ^ ")") statements_rev) remaining_values rest
  in
...
let render_format_program = fun body literal_text format_items rendered_args ->
  match render_builder_program
    ...
    format_items
    rendered_args with
  | Error message -> Macro_result.error
    (Macro_error.make ?span:(Macro_token_stream.span body) message)
  | Ok source -> Macro_result.ok_source source

let expand = fun body ->
  match Macro_parse.expr_arguments body with
  | Error err ->
      Macro_result.error err
  | Ok [] ->
      Macro_result.error
        (Macro_error.make ?span:(Macro_token_stream.span body) "format! requires a format string argument")
  | Ok (format_arg :: value_args) ->
      ...
      if Syn.Ceibo.Red.SyntaxNode.kind format_arg != Syn.SyntaxKind.STRING_LITERAL then
        Macro_result.error
          (Macro_error.make ?span:(Macro_token_stream.span_of_node format_arg) "format! currently requires a string literal as its first argument")
      else
        ...
        | None -> Macro_result.error
          (Macro_error.make ?span:(Macro_token_stream.span body) "format! could not recover the format string span")
        | Some span -> (
...
let provider = fun () ->
  Macro_provider.v ~module_path:[ "Macro" ] [ Macro_provider.fn "format" expand; ]
